...
import { deleteMeetiAction } from "../actions/meeti.action";
import toast from "react-hot-toast";
import { redirect } from "next/navigation";

export default function DeleteMeetiDialog() {
  const { open, setOpen, meeti, setMeeti } = useMeetiStore();
  if (!meeti) return null;

  const handleClick = async () => {
    const { error, success } = await deleteMeetiAction(meeti.id);
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-gray-600">
                      ¿Confirmas que deseas eliminar este Meeti? Un Meeti
                      Eliminado no se puede recuperar
                    </p>
